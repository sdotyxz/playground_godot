...
@export var zoom_in_key : Key = KEY_Q
@export var zoom_out_key : Key = KEY_E
...
func _unhandled_input(event):
	handle_mobile_gestures(event)
	handle_mouse_button(event)
	handle_mouse_motion(event)
	handle_input_key(event)

func handle_mobile_gestures(event):
